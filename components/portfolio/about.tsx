...
const icons = [Server, Database, Code2, LineChart]

export function About() {
  const { t } = useLanguage()
  const stats: { value: string; label: string }[] = t("about.stats")
  const highlightsFlat: string[] = t("about.highlights")

  // Le tableau JSON alterne [titre, description, titre, description, ...]
  const expertise = icons.map((icon, i) => ({
    icon,
    title: highlightsFlat[i * 2],
    desc: highlightsFlat[i * 2 + 1],
  }))
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="text-primary font-medium mb-6">{t("about.role")}</p>
