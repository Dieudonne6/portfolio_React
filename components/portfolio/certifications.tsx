...
// Styling / liens : ne dépendent pas de la langue, on les garde ici
const certStyles = [
  {
    link: "/Certifications/google_analytic_4_certificate.png",
    bgColor: "from-blue-500/20 to-blue-600/10",
...
type CertItem = { name: string; issuer: string; date: string; id: string }

export function Certifications() {
  const { t } = useLanguage()
  const items: CertItem[] = t("certifications.items")
  const certifications = items.map((item, i) => ({ ...item, ...certStyles[i] }))
          ...
          {certifications.map((cert) => (
            ...
            >
              ...
              <div className="relative p-6">
                ...
                <p className="text-muted-foreground text-sm mb-4">
                  {t("certifications.issuedBy")} <span className="text-foreground font-medium">{cert.issuer}</span>
                </p>
